...
$result = $conexion->query("SELECT * FROM usuario WHERE id = " . (int)$id);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errores)) {
        $stmt = $conexion->prepare("UPDATE usuario SET nombre = ?, correo = ?, admin = ? WHERE id = ?");
...
<div class="flex-1 p-8">
    <h2 class="text-3xl font-bold text-purple-main mb-6">Editar Usuario</h2>
    ...
    <form method="POST" class="bg-white p-6 rounded shadow max-w-lg space-y-4">
        <input name="nombre" maxlength="100" class="w-full p-2 border rounded" placeholder="Nombre" required value="<?= htmlspecialchars($_POST['nombre'] ?? $usuario['nombre']) ?>">
        <input name="correo" type="email" class="w-full p-2 border rounded" placeholder="Correo" required value="<?= htmlspecialchars($_POST['correo'] ?? $usuario['correo']) ?>">

        <label class="flex items-center space-x-2">
            <input type="checkbox" name="admin" <?= (($usuario['admin'] ?? 0) == 1) ? 'checked' : '' ?> class="w-4 h-4">
